...
async function fetchApi(params: Record<string, string>): Promise<any> {
  ...
  if (type === "lrc") {
    // netease/kuwo 走 GD API
    if (server === "netease" || server === "kuwo") {
    ...
    // tencent/kugou 走 i-meto lrc
    try {
      const url = `https://api.i-meto.com/meting/api?server=${server}&type=lrc&id=${encodeURIComponent(id)}`;
      const resp = await fetch(url);
...
export async function resolveOnlineAudioUrl(platform: string, id: string): Promise<string> {
  ...
  // tencent/kugou 走 i-meto（可能404，audio error 后自动切下一首）
  return `https://api.i-meto.com/meting/api?server=${platform}&type=url&id=${encodeURIComponent(id)}`;
}
